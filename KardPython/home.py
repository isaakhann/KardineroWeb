...
def upload_file():
    ...
    if file:
        ...
        ecg_data_global = parser.read_scp_file(filepath)
...
@app.route('/show_patient_info', methods=['GET'])
def show_patient_info():
    print("Starting to fetch patient info...")
    parser = SCPParser()
    
    file_path = '/Users/isakhan/Documents/KardPython/uploads/rest.scp'
    print(f"File path: {file_path}")
    
    patient_data = parser.get_patient_info(file_path)
    
    if patient_data and not patient_data.get("error"):
        print(f"Patient data found: {patient_data}")
        return render_template('patient_info.html', patient_data=patient_data)
    else:
        print("No patient info found or error occurred.")
        return render_template('patient_info.html', error="No patient info found.")
...
def parse_scp_file(file_path):
    try:
        ...
        patient_info = scp_parser.get_patient_info(file_path)
        
        if not patient_info or "error" in patient_info:
            raise ValueError("Failed to extract patient information.")
        
        # Combine first and last names for full name
        full_name = f"{patient_info.get('first_name', '')} {patient_info.get('last_name', '')}".strip()
        ...
        csv_string = convert_to_csv(full_name, full_ecg_data)
...
def extract_first_name(full_name):
    # Assuming the full_name is already a string
    name_components = full_name.split()
    if len(name_components) > 0:
        return name_components[0]  # Return the first name (first word)
    return "Patient"
